...
router.get('/', (req, res) => {
    res.json(db.prepare('SELECT * FROM content_items WHERE workspace = ? ORDER BY created_at DESC').all(req.workspace));
});
...
router.post('/', (req, res) => {
    ...
    const info = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    `).run(title.trim(), channel || 'reseaux-sociaux', status || 'brouillon', body || '', Date.now(), req.workspace);
    res.status(201).json(db.prepare('SELECT * FROM content_items WHERE id = ?').get(info.lastInsertRowid));
});

router.put('/:id', (req, res) => {
    const existing = db.prepare('SELECT * FROM content_items WHERE id = ? AND workspace = ?').get(req.params.id, req.workspace);
...
router.delete('/:id', (req, res) => {
    db.prepare('DELETE FROM content_items WHERE id = ? AND workspace = ?').run(req.params.id, req.workspace);
    res.status(204).end();
});
...
router.post('/:id/publish', async (req, res) => {
    const item = db.prepare('SELECT * FROM content_items WHERE id = ? AND workspace = ?').get(req.params.id, req.workspace);
    ...
    try {
        ...
        `).run(`Contenu publié : ${item.title}`, Date.now(), req.workspace);
        res.json(db.prepare('SELECT * FROM content_items WHERE id = ?').get(item.id));
    } catch (error) {
        db.prepare(`
            INSERT INTO activity_log (agent_id, agent_name, color, text, model, status, timestamp, workspace)
            VALUES ('cmo', 'CMO', 'cmo', ?, 'webhook', 'FAILED', ?, ?)
        `).run(`Échec de publication : ${item.title}`, Date.now(), req.workspace);
